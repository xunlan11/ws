...
int main(int argc, char **argv) {
    ...
    if (argc < 3 || argc > 4) {
        cerr << endl
             << "Usage: ./stereo_inertial_realsense_D455 path_to_vocabulary path_to_settings (trajectory_file_name)"
             << endl;
        return 1;
    }
    ...
    for (rs2::sensor sensor : sensors)
        if (sensor.supports(RS2_CAMERA_INFO_NAME)) {
            ...
            if (index == 1) {
                ...
                sensor.set_option(RS2_OPTION_EMITTER_ENABLED, 0); // 关闭发射器以避免干扰
                ...
                if (sensor.supports(RS2_OPTION_DEPTH_UNITS))
                    sensor.set_option(RS2_OPTION_DEPTH_UNITS, 0.001f);
            }
            ...
            if (index == 2){
                ...
                // D455 颜色相机特有设置
                if (sensor.supports(RS2_OPTION_BRIGHTNESS))
                    sensor.set_option(RS2_OPTION_BRIGHTNESS, 0);
                if (sensor.supports(RS2_OPTION_CONTRAST))
                    sensor.set_option(RS2_OPTION_CONTRAST, 50);
            }

            if (index == 3){
                // D455 IMU设置
                sensor.set_option(RS2_OPTION_ENABLE_MOTION_CORRECTION, 0); // 关闭运动校正以获取原始数据
                
                // D455支持的IMU特有选项
                if (sensor.supports(RS2_OPTION_GLOBAL_TIME_ENABLED))
                    sensor.set_option(RS2_OPTION_GLOBAL_TIME_ENABLED, 1);
                
                // 设置IMU频率（D455支持200Hz IMU采样率）
                if (sensor.supports(RS2_OPTION_FRAMES_QUEUE_SIZE))
                    sensor.set_option(RS2_OPTION_FRAMES_QUEUE_SIZE, 32); // 增加队列大小以处理高帧率
            }
        }
    ...
    // 配置D455 IMU流 - 确保完整使用200Hz的能力
    cfg.enable_stream(RS2_STREAM_ACCEL, RS2_FORMAT_MOTION_XYZ32F, 200); // 指定200Hz
    cfg.enable_stream(RS2_STREAM_GYRO, RS2_FORMAT_MOTION_XYZ32F, 200);  // 指定200Hz
