...
int main(int argc, char **argv) {
    ...
    if (argc < 3 || argc > 4) {
        cerr << endl
             << "Usage: ./stereo_realsense_D455 path_to_vocabulary path_to_settings (trajectory_file_name)"
             << endl;
        return 1;
    }
    ...
    for (rs2::sensor sensor : sensors)
        if (sensor.supports(RS2_CAMERA_INFO_NAME)) {
            ...
            if (index == 1) {
                ...
                sensor.set_option(RS2_OPTION_EMITTER_ENABLED, 0); // 关闭发射器
                
                // D455特有的激光功率控制（如果支持）
                if (sensor.supports(RS2_OPTION_LASER_POWER)) {
                    sensor.set_option(RS2_OPTION_LASER_POWER, 0); // 关闭激光以避免干扰
                }
                
                // D455特有的参数（如果需要）
                if (sensor.supports(RS2_OPTION_DEPTH_UNITS)) {
                    sensor.set_option(RS2_OPTION_DEPTH_UNITS, 0.001f); // 1mm深度单位
                }
            }
    ...
    // D455 支持的分辨率和帧率（确保设置与相机兼容）
    // 默认保留640x480@30fps，如果需要可以更改为其他分辨率如848x480
    cfg.enable_stream(RS2_STREAM_INFRARED, 1, 640, 480, RS2_FORMAT_Y8, 30);
    cfg.enable_stream(RS2_STREAM_INFRARED, 2, 640, 480, RS2_FORMAT_Y8, 30);
    
    // 如果想要使用深度信息，可以取消下面这行的注释
